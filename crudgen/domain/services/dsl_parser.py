...
from crudgen.domain.models import IdMeta, Relation
from crudgen.domain.models import Entity, Field
from crudgen.domain.services import TypeResolver


def parse_spec_data(spec_data: dict, type_resolver: Optional[TypeResolver]) -> List[Entity]:
    entities = []
...
def parse_entity_data(name: str, data: dict, generated_types: Set[str],
    ...
    fields_data.pop(SpecConfig.META)
...
def parse_field_id_meta(data: dict) -> Optional[IdMeta]:
    id_data = data[SpecConfig.ID]
    if not id_data: return None

    if type(id_data) is str:
        return IdMeta(IdMeta.GenerationType(id_data))

    id_meta = IdMeta(IdMeta.GenerationType(id_data[SpecConfig.TYPE]))
    id_meta.sequence = id_data.get(SpecConfig.SEQUENCE)


def parse_field_relation(data: dict) -> Optional[Relation]:
    relation_data = data[SpecConfig.RELATION]
    if not relation_data: return None

    relation = Relation(Relation.RelationType(relation_data[SpecConfig.TYPE]))
